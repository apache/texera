...
class TexeraOtelLogAppenderSpec extends AnyFlatSpec with Matchers {
  ...
  it should "redact secrets inside an attached stack trace, not just the message" in {
    val (_, exporter, appender) = newFixture()
    val ctx = LoggerFactory.getILoggerFactory.asInstanceOf[LoggerContext]
    val logger = ctx.getLogger("test.logger").asInstanceOf[Logger]
    val boom = new RuntimeException("db connect failed for password=hunter2")
    val ev = new LoggingEvent("fqcn", logger, Level.ERROR, "operation failed", boom, null)
    appender.doAppend(ev)

    val body = exporter.getFinishedLogRecordItems.asScala.head.getBodyValue.asString
    body should include("[REDACTED]")
    body should not include "hunter2"
    // The stack trace's newlines are preserved (only the message is C0-stripped).
    body should include("\n")
  }
  ...
  // ----- security: MDC deny-list ----------------------------------------

  private def attrsOf(record: io.opentelemetry.sdk.logs.data.LogRecordData): Map[String, String] =
    record.getAttributes.asMap.asScala.iterator.map { case (k, v) => k.getKey -> v.toString }.toMap

  it should "forward arbitrary correlation MDC keys (deny-list, not allow-list)" in {
    val (_, exporter, appender) = newFixture()
    appender.doAppend(
      makeEvent(
        "msg",
    ...
    attrs.keySet should contain allOf ("trace_id", "texera.workflow.id", "some.new.key")
    attrs("some.new.key") shouldBe "kept"
  }

  it should "drop the noisy Pekko bridge MDC keys" in {
    val (_, exporter, appender) = newFixture()
    val denied = LogSanitizer.DeniedMdcKeys.iterator.map(_ -> "noise").toMap
    appender.doAppend(makeEvent("msg", mdc = denied + ("trace_id" -> "abc")))

    val attrs = attrsOf(exporter.getFinishedLogRecordItems.asScala.head)
    attrs.keySet should contain("trace_id")
    attrs.keySet should contain noElementsOf LogSanitizer.DeniedMdcKeys
  }

  it should "redact a secret-shaped MDC value while keeping its key" in {
    val (_, exporter, appender) = newFixture()
    appender.doAppend(makeEvent("msg", mdc = Map("authz" -> "Bearer abc123XYZ.foo")))

    val attrs = attrsOf(exporter.getFinishedLogRecordItems.asScala.head)
    attrs.keySet should contain("authz")
    attrs("authz") should include("[REDACTED]")
    attrs("authz") should not include "abc123XYZ"
  }

  it should "redact the value of a credential-named MDC key even when the value looks benign" in {
    val (_, exporter, appender) = newFixture()
    appender.doAppend(makeEvent("msg", mdc = Map("password" -> "p4ssw0rd", "api_key" -> "plain")))
