...
class TexeraOtelLogAppender extends UnsynchronizedAppenderBase[ILoggingEvent] {
  ...
  private def emit(logger: Logger, event: ILoggingEvent): Unit = {
    // Control-strip the message only (trace newlines must survive), then append
    ...
    val body = LogSanitizer.truncate(LogSanitizer.redactSecrets(combined))
    val builder = logger
      .logRecordBuilder()
      .setBody(body)
    ...
    // Surviving (deny-list filtered) MDC keys as typed attributes.
