...
public class SelectExtractStatement extends Statement {
     ...
     */
    private String getExtractionID(){
    ...
    public List<OperatorLinkBean> getInternalLinkBeans(){
        return Arrays.asList(
                   new OperatorLinkBean(getSelectionID(), getOutputID()),
                   new OperatorLinkBean(getExtractionID(), getSelectionID()),
                   new OperatorLinkBean(getInputID(), getExtractionID())
               );
    }
    ...
    public List<OperatorBean> getInternalOperatorBeans(){
        ...
        operators.add(new PassThroughBean(getOutputID(), "PassThrough"));
        // Build and append bean for Projection
        if(this.selectPredicate==null){
            operators.add(new PassThroughBean(getSelectionID(), "PassThrough"));
        }else{
            operators.add(this.selectPredicate.getOperatorBean(getSelectionID()));
        }
        // Build and append bean for Extraction predicate
        if(this.extractPredicate==null){
            operators.add(new PassThroughBean(getExtractionID(), "PassThrough"));
        }else{
            operators.add(this.extractPredicate.getOperatorBean(getExtractionID()));
        }
        // Build and append bean for the Source
        operators.add(new PassThroughBean(getInputID(), "PassThrough"));
    ...
    public boolean equals(Object other) {
        ...
        SelectExtractStatement selectStatement = (SelectExtractStatement) other;
