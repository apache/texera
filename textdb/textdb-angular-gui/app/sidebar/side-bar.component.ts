...
export class SideBarComponent {
    ...
    selectorList: string[] = ["dictionaryEntries", "password", "matchingType",
        ...
        "aggregationType", "attributes", "tableName", "attribute", "keywordQuery", "language"].concat(this.hiddenList);
    ...
    constructor(private currentDataService: CurrentDataService) {
        currentDataService.newAddition$.subscribe(
            data => {
                ...
                if (data.operatorData.properties.attributes.keywordQuery) {
                    this.twitterQuery = data.operatorData.properties.attributes.keywordQuery;
                }
                if (data.operatorData.properties.attributes.language) {
                    this.twitterLanguage = data.operatorData.properties.attributes.language;
                }
    ...
    twitterQueryManuallyAdded(event: string) {
        ...
        this.data.properties.attributes.keywordQuery = this.twitterQuery;
        this.onFormChange("keywordQuery");
    }

    twitterLanguageManuallyAdded(event: string) {
        if (event.length === 0) {
            this.twitterLanguage = [];
        } else {
            this.twitterLanguage = event.split(",");
        }
        this.data.properties.attributes.language = this.twitterLanguage;
        this.onFormChange("language");
    }
