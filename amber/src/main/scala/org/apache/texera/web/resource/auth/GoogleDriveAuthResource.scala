...
object GoogleDriveAuthResource {
    ...
    // def getRefreshToken(user: User): Option[String] = {
...
class GoogleDriveAuthResource {
    ...
    final val STATUS_NO_REFRESH_TOKEN = "no_refresh_token"                                                                                                           
    final val STATUS_INVALID_GRANT = "invalid_grant"
    ...
    def getDriveAccessToken(@Auth sessionUser: SessionUser): Response = {
        ...
        try{
            ...
            return Response.ok(DriveTokenIssueResponse(STATUS_OK, Some(accessToken))).build()
        } catch {
            case e: TokenResponseException =>
                if (e.getDetails != null && e.getDetails.getError == STATUS_INVALID_GRANT) {
                    return Response.ok(DriveTokenIssueResponse(STATUS_INVALID_GRANT, None)).build()
                } else {
    ...
    // def startOAuthFlow(@Auth sessionUser: SessionUser): Response = {
