...
class PveWebsocketResourceSpec extends AnyFlatSpec with Matchers with MockFactory {
  ...
  /**
    * Where `PveManager` looks for a PVE's interpreter. Unconditional `bin/python`, mirroring
    * `PveManager.pythonBinPath` on this branch: the Windows `Scripts/python.exe` arm is a
    * later change to `PveManager` that this backport does not carry, and hardcoding it here
