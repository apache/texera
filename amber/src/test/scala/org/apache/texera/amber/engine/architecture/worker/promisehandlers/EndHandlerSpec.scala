...
class EndHandlerSpec extends AnyFlatSpec {
  ...
  /**
    * A reply to one of this worker's own requests is not work, and the controller cannot order
    ...
    * controller when that advance runs — `workerExecutionCompleted`, emitted right after the last
    * `portCompleted` — is replied to afterwards, and the controller selects input channels out of
