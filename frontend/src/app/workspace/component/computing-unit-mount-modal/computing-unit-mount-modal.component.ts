 ...
 * (picked with the shared dataset-selection modal) and see everything currently mounted.
 * Adding proxies through the computing-unit service to that CU's node mounter; state lives
 * on the mounter, so the list is always re-fetched. Mounts are released when the CU pod is
 * deleted (the mounter's pod watcher), so there is no per-dataset unmount action here.
 */
@UntilDestroy()
@Component({
