...
class RandomKSamplingOpExec(descString:String, idx: Int, workerCount:Int)
    extends FilterOpExec {
    private val desc: RandomKSamplingOpDesc = objectMapper.readValue(descString, classOf[RandomKSamplingOpDesc])
