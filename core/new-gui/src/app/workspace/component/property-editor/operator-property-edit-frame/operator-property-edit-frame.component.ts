...
export class OperatorPropertyEditFrameComponent implements OnInit, OnChanges, OnDestroy {
  ...
  updateOperatorPropertiesOnSchemaChange(
    ...
    console.log(oldSchema, newSchema, oldInputSchema, newInputSchema);
    if (!oldSchema.jsonSchema.properties)
      return;
    ...
    for (let port = 0; port < oldInputSchema.length; ++port) {
      const attributeMapping = levenshtein(
        ...
        (a: SchemaAttribute, b: SchemaAttribute) => {
          if (a.attributeName === b.attributeName && a.attributeType === b.attributeType)
            return 0;
          else if (a.attributeName !== a.attributeName && a.attributeType !== b.attributeType)
            return 2;
          else 
            return 1;
        }
      );
      const pairs: SchemaAttribute[][] = attributeMapping.pairs();
      const resolvedJsonSchema = jsonRefLite.resolve(oldSchema.jsonSchema);
      console.log("resolved", resolvedJsonSchema)
      const newProperty = this.updateOperatorProperty(operator.operatorProperties, pairs, resolvedJsonSchema.properties, port);
      this.workflowActionService.setOperatorProperty(operatorID, (newProperty));
      console.log("newOperator", this.workflowActionService.getTexeraGraph().getOperator(operatorID))
    }
  ...
  private updateOperatorProperty(currentProperties: Readonly<{ [key: string]: any; }>, mapping: SchemaAttribute[][], jsonSchemaProperties: { [key: string]: boolean | CustomJSONSchema7; }, port: number) {
    console.log("properties: ", currentProperties);
    console.log("mapping: ", mapping);
    const updatePropertyRecurse = (properties: any, jsonSchema: { [key: string]: CustomJSONSchema7 | boolean; } | undefined) => {
      ...
      for (const key in jsonSchema) {
        if (typeof jsonSchema[key] === "boolean")
          continue;
        const prop = jsonSchema[key] as CustomJSONSchema7;
        if (prop.autofillAttributeOnPort === port) {
          if (prop.type === "array") {
            // TODO
            let newArray = [];
            ...
            for (var i = 0; i < mapping.length; ++i) {
              ...
              if (!pair[0] || properties[key] === pair[0].attributeName && pair[1]) {
                properties[key] = pair[1].attributeName;
              }
            }
          }
        } else {
          if ((jsonSchema[key] as CustomJSONSchema7).type === "object") {
            updatePropertyRecurse(properties[key], prop.properties);
          } else if ((jsonSchema[key] as CustomJSONSchema7).type === "array") {
            for (const index in properties[key]) {
              console.log("items", prop.items);
              updatePropertyRecurse(properties[key][index], (prop.items as CustomJSONSchema7).properties)
            }
          }
        }
      }
