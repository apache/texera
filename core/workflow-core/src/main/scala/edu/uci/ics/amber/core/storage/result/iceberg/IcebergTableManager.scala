...
class IcebergTableManager(table: Table) {
  ...
  private def detectNumericFields(): Set[String] = {
    schema
      ...
      .collect {
        case col
            if col.`type`() == Types.IntegerType.get() ||
  ...
  private def detectDateFields(): Set[String] = {
    schema
      ...
      .collect {
        case col
            if col.`type`() == Types.TimestampType.withoutZone() ||
  ...
  private def detectCategoricalFields(): Set[String] = {
    schema
      ...
      .collect {
        case col
            if col.`type`() == Types.StringType.get() || col.`type`() == Types.BooleanType.get() =>
          col.name()
      }
      .toSet
  }
  ...
  def calculateNumericStats(): Map[String, Map[String, Double]] = {
    ...
    stats.map {
      case (field, fieldStats) =>
        field -> Map(
          "min" -> fieldStats("min"),
          "max" -> fieldStats("max"),
          "mean" -> (if (fieldStats("count") > 0) fieldStats("sum") / fieldStats("count")
                     else Double.NaN)
        )
    }.toMap
  }
    ...
    sampleDocuments().foreach { record =>
      dateFields.foreach { field =>
        ...
        if (value.isDefined) {
          val fieldStats = stats(field)
          fieldStats("min") =
            if (value.get.before(fieldStats("min"))) value.get else fieldStats("min")
          fieldStats("max") =
            if (value.get.after(fieldStats("max"))) value.get else fieldStats("max")
        }
      }
    }
