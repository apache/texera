...
class CartesianProductOpExecSpec extends AnyFlatSpec with BeforeAndAfter {
  ...
  it should "work with basic two input streams with no duplicate attribute names" in {
    ...
    (1 to numLeftTuples).map(value => {
      assert(
        opExec
          .processTexeraTuple(Left(generate_tuple(leftSchema, Some(value))), leftPort)
          .isEmpty
      )
    })
    assert(opExec.processTexeraTuple(Right(InputExhausted()), leftPort).isEmpty)

    // process 5 right tuples
    val outputTuples: List[Tuple] = (numLeftTuples + 1 to numLeftTuples + numRightTuples)
      .map(value =>
        opExec
          .processTexeraTuple(Left(generate_tuple(rightSchema, Some(value))), rightPort)
      )
      .foldLeft(Iterator[Tuple]())(_ ++ _)
      .toList
    assert(opExec.processTexeraTuple(Right(InputExhausted()), rightPort).isEmpty)
