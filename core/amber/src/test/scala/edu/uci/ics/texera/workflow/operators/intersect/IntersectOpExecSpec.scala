...
class IntersectOpExecSpec extends AnyFlatSpec with BeforeAndAfter {
  ...
  it should "work with one empty input upstream after a data stream" in {
    ...
    (1 to 100).map(_ => {
      opExec.processTexeraTuple(Left(tuple()), input0)
      opExec.processTexeraTuple(
        Left(commonTuples(Random.nextInt(commonTuples.size))),
        input0
      )
    })
    assert(opExec.processTexeraTuple(Right(InputExhausted()), input0).isEmpty)
  ...
  it should "work with one empty input upstream after a data stream - other order" in {
    ...
    (1 to 100).map(_ => {
      opExec.processTexeraTuple(Left(tuple()), input1)
      opExec.processTexeraTuple(
        Left(commonTuples(Random.nextInt(commonTuples.size))),
        input1
      )
  ...
  it should "work with two empty input upstreams" in {
    ...
    opExec.open()
    assert(opExec.processTexeraTuple(Right(InputExhausted()), 0).isEmpty)
    assert(opExec.processTexeraTuple(Right(InputExhausted()), 1).isEmpty)
    opExec.close()
  }
