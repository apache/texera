...
class SpecializedFilterOpExecSpec extends AnyFlatSpec with BeforeAndAfter {
  ...
  it should "throw when predicates is null" in {
    ...
    assertThrows[NullPointerException] {
      opExec.processTexeraTuple(Left(allNullTuple), inputPort)
    }
    opExec.close()
  }
  ...
  it should "do nothing when predicates is an empty list" in {
    ...
    opExec.open()
    assert(opExec.processTexeraTuple(Left(allNullTuple), inputPort).isEmpty)
    opExec.close()
  }
  ...
  it should "output null tuples when filtering is_null" in {
    tuplesWithOneFieldNull
      .map(nullTuple => {
        ...
        opExec.open()
        assert(!opExec.processTexeraTuple(Left(nullTuple), inputPort).isEmpty)
        opExec.close()
      })
  }
  ...
  it should "filter out non null tuples when filtering is_null" in {
    ...
    opExec.open()
    assert(opExec.processTexeraTuple(Left(nonNullTuple), inputPort).isEmpty)
    opExec.close()
  }
