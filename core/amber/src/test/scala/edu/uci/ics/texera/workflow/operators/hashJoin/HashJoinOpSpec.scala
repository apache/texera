...
class HashJoinOpSpec extends AnyFlatSpec with BeforeAndAfter {
  ...
  it should "work with basic two input streams with different buildAttributeName and probeAttributeName" in {
    ...
    (0 to 7).map(i => {
      assert(
        buildOpExec.processTexeraTuple(Left(tuple("build", 1, Some(i))), build).isEmpty
      )
    })

    val buildOpOutputIterator =
      buildOpExec.processTexeraTuple(Right(InputExhausted()), build)
    ...
    while (buildOpOutputIterator.hasNext) {
      assert(
        probeOpExec
          .processTexeraTuple(Left(buildOpOutputIterator.next()), build)
          .isEmpty
      )
    }
    assert(probeOpExec.processTexeraTuple(Right(InputExhausted()), build).isEmpty)

    buildOpExec.close()

    val outputTuples = (5 to 9)
      .map(i => probeOpExec.processTexeraTuple(Left(tuple("probe", 1, Some(i))), probe))
      .foldLeft(Iterator[Tuple]())(_ ++ _)
      .toList

    assert(probeOpExec.processTexeraTuple(Right(InputExhausted()), probe).isEmpty)
  ...
  it should "work with basic two input streams with the same buildAttributeName and probeAttributeName" in {
    ...
    (0 to 7).map(i => {
      assert(
        buildOpExec.processTexeraTuple(Left(tuple("same", 1, Some(i))), build).isEmpty
      )
    })
    val buildOpOutputIterator =
      buildOpExec.processTexeraTuple(Right(InputExhausted()), build)
    ...
    val outputTuples = (5 to 9)
      .map(i => probeOpExec.processTexeraTuple(Left(tuple("same", n = 2, Some(i))), probe))
      .foldLeft(Iterator[Tuple]())(_ ++ _)
      .toList
  ...
  it should "work with basic two input streams with the same buildAttributeName and probeAttributeName with Full Outer Join" in {
    ...
    assert(
      (5 to 9)
        .map(_ => {
          probeOpExec.processTexeraTuple(Left(tuple("same", n = 2, None)), probe)
        })
        .foldLeft(Iterator[Tuple]())(_ ++ _)
        .size == 5
    )

    assert(probeOpExec.processTexeraTuple(Right(InputExhausted()), probe).size == 8)
