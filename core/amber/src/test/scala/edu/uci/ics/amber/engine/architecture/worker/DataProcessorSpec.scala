...
import edu.uci.ics.amber.engine.common.ambermessage.{ChannelID, DataFrame, EndOfUpstream, WorkflowFIFOMessage}
...
class DataProcessorSpec extends AnyFlatSpec with MockFactory with BeforeAndAfterEach {
  ...
  private val link = PhysicalLink(upstreamOp.id, 0, testOp.id, 0)
