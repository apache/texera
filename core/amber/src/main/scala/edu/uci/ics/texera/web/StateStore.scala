...
class StateStore[T](defaultState: T) {
  ...
  class SyncableState{
    ...
    def subscribe(callback: Iterable[TexeraWebSocketEvent] => Unit): Subscription = {
      withLock {
        val events = onChangedHandlers.values.flatMap(handler => handler(defaultState, currentState))
        callback(events)
        callbackSubject.subscribe(callback)
      }
    }

    def registerStateChangeHandler(handler:(T,T) => Iterable[TexeraWebSocketEvent]):Subscription = {
