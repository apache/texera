...
import edu.uci.ics.texera.web.model.jooq.generated.tables.pojos.{Project, Workflow}
...
class ProjectSearchQueryBuilder extends SearchQueryBuilder {
  ...
  override protected def constructGroupByClause(user: SessionUser, params: DashboardResource.SearchQueryParams): Seq[GroupField] = Seq(PROJECT.PID)

  override protected def getOrderFields(user: SessionUser, params: DashboardResource.SearchQueryParams): Seq[OrderField[_]] = FulltextSearchQueryUtils.getOrderFields(SearchQueryBuilder.PROJECT_RESOURCE_TYPE, params)
