...
object DatasetFileNode {
  def fromPhysicalFileNodes(
    ...
    map.foreach { case ((ownerEmail, datasetName, versionName), physicalNodes) =>
      val ownerNode = ownerNodes.getOrElseUpdate(ownerEmail, {
  ...
  private def addNodeToTree(
    ...
    while (!queue.isEmpty) {
      ...
      val fileType = if (Files.isDirectory(currentPhysicalNode.getAbsolutePath)) "directory" else "file"
      val existingNode = currentParent.getChildren.find(child => child.getName == nodeName && child.getNodeType == fileType)
