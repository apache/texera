...
import edu.uci.ics.amber.engine.architecture.worker.DataProcessor.{DPOutputIterator, FinalizeLink, FinalizeOperator, ForLoopIterationCompleted}
...
object DataProcessor {
  ...
  case class ForLoopIterationCompleted() extends SpecialDataTuple
...
class DataProcessor(
  ...
  private[this] def outputOneTuple(): Unit = {
    ...
    outputTuple match {
      case ForLoopIterationCompleted() =>
        // engine should notify controller ..
