...
/**
  * A cost estimator should estimate a cost of running a region under the given resource units.
  */
trait CostEstimator {
  ...
  override def estimate(region: Region, resourceUnits: Int): Double = {
    this.operatorEstimatedTimeOption match {
      case Some(operatorEstimatedTime) =>
        ...
        val longestRunningOpExecutionTime =
          if (opExecutionTimes.nonEmpty) opExecutionTimes.max else 0.0
