package edu.uci.ics.texera.workflow.operators.visualization;

public class VisualizationConstants {
    public static final String BAR = "bar";

    public static final String LINE = "line";
    public static final String SPLINE = "spline";

    public static final String PIE = "pie";
    public static final String DONUT = "donut";
    public static final String HTML_VIZ = "HTML visualizer";
    public static final String SIMPLE_SCATTERPLOT = "scatter";
    public static final String SPATIAL_SCATTERPLOT = "spatial scatterplot";
    public static final String WORD_CLOUD = "word cloud";
    public static final String LINEAR_BOXPLOT = "linear";
    public static final String INCLUSIVE_BOXPLOT = "inclusive";
    public static final String EXCLUSIVE_BOXPLOT = "exclusive";

}
