...
trait ModifyLogicHandler {
  ...
  override def modifyLogic(msg: ModifyLogicRequest, ctx: AsyncRPCContext): Future[EmptyReturn] = {
    msg.updateRequest.find(
      _.targetOpId == VirtualIdentityUtils.getPhysicalOpId(this.actorId)
    ) match {
      case Some(value) =>
  ...
  private def performUpdateExecutor(req: UpdateExecutorRequest): Unit = {
    ...
    if (req.stateTransferFunc.nonEmpty) {
      ...
      val stateTransferFunc: StateTransferFunc =
        AmberRuntime.serde.deserialize(bytesForStateTransfer, classOf[StateTransferFunc]).get
