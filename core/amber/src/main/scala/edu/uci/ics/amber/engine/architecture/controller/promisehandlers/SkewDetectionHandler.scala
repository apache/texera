      ...
      cp.workflow.physicalPlan.operators.foreach(physicalOp => {
        if (physicalOp.isHashJoinOperator) {
          ...
          skewedAndHelperPairsForFirstPhase.foreach(skewedHelperAndReplication =>
            logger.info(
              s"Reshape ${cp.workflow.executionId.id} #${workflowReshapeState.detectionCallCount}: First phase process begins - Skewed ${skewedHelperAndReplication._1} :: Helper ${skewedHelperAndReplication._2} - Replication/Intimation required: ${skewedHelperAndReplication._3}"
            )
          )
                ...
                stateTransferOrIntimationSuccessful => {
                  if (stateTransferOrIntimationSuccessful) {
                    workflowReshapeState.skewedToStateTransferOrIntimationDone(currSkewedWorker) =
                      true
                    logger.info(
                      s"Reshape ${cp.workflow.executionId.id} #${workflowReshapeState.detectionCallCount}: State transfer/intimation completed - $currSkewedWorker to $currHelperWorker"
                    )
                    implementFirstPhasePartitioning(
                      ...
                      logger.info(
                        s"Reshape ${cp.workflow.executionId.id} #${workflowReshapeState.detectionCallCount}: First phase request finished for $currSkewedWorker to $currHelperWorker"
                      )
                      firstPhaseFinishedCount += 1
                      if (firstPhaseFinishedCount == skewedAndHelperPairsForFirstPhase.size) {
                        logger.info(
                          s"Reshape ${cp.workflow.executionId.id} #${workflowReshapeState.detectionCallCount}: First phase requests completed for ${skewedAndHelperPairsForFirstPhase.size} pairs"
                        )
                        workflowReshapeState.firstPhaseRequestsFinished = true
                      }
                    })
          ...
          skewedAndHelperPairsForSecondPhase.foreach(skewedAndHelper =>
            logger.info(
              s"Reshape ${cp.workflow.executionId.id} #${workflowReshapeState.detectionCallCount}: Second phase request begins - Skewed ${skewedAndHelper._1} :: Helper ${skewedAndHelper._2}"
            )
          )
          ...
          Future
            ...
            .onSuccess(_ => {
              workflowReshapeState.secondPhaseRequestsFinished = true
              logger.info(
                s"Reshape ${cp.workflow.executionId.id} #${workflowReshapeState.detectionCallCount}: Second phase requests completed for ${skewedAndHelperPairsForSecondPhase.size} pairs"
              )
            })
          ...
          skewedAndHelperPairsForPauseMitigationPhase.foreach(skewedAndHelper =>
            logger.info(
              s"Reshape ${cp.workflow.executionId.id} #${workflowReshapeState.detectionCallCount}: Pause Mitigation phase request begins - Skewed ${skewedAndHelper._1} :: Helper ${skewedAndHelper._2}"
            )
          )
          ...
          Future
            ...
            .onSuccess(_ => {
              workflowReshapeState.pauseMitigationRequestsFinished = true
              logger.info(
                s"Reshape ${cp.workflow.executionId.id} #${workflowReshapeState.detectionCallCount}: Pause Mitigation phase requests completed for ${skewedAndHelperPairsForPauseMitigationPhase.size} pairs"
              )
            })
