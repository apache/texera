...
class UnnestStringOpExec(opDesc: UnnestStringOpDesc)
  ...
  def splitByDelimiter(tuple: Tuple): Iterator[Tuple] = {
    ...
    val outputSchema = opDesc.operatorInfo.outputPorts.map(outputPort => opDesc.outputPortToSchemaMapping(outputPort.id)).head
