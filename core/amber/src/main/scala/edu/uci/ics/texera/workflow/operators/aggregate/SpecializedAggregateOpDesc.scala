...
class SpecializedAggregateOpDesc extends AggregateOpDesc {
  ...
  override def operatorInfo: OperatorInfo =
    OperatorInfo(
      ...
      inputPorts = List(InputPort("in")),  // a hacky solution to have unique port names for reference purpose
      outputPorts = List(OutputPort("out"))  // a hacky solution to have unique port names for reference purpose
    )
