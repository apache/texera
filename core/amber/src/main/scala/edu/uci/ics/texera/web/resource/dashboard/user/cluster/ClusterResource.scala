...
import edu.uci.ics.texera.web.resource.dashboard.user.cluster.ClusterResource.{ERR_USER_HAS_NO_ACCESS_TO_CLUSTER_MESSAGE, clusterActivityDao, clusterDao, context}
...
class ClusterResource {
  ...
  private def updateClusterActivityEndTime(clusterId: Int): Unit = {
    context.update(CLUSTER_ACTIVITY)
      ...
      .and(CLUSTER_ACTIVITY.START_TIME.eq(
        context.select(max(CLUSTER_ACTIVITY.START_TIME))
