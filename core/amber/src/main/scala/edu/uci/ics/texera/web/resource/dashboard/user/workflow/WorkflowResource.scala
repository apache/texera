    ...
    try {
      ...
      val clickableFileEntry =
        resourceType match {
          ...
          case "" => workflowQuery.union(projectQuery).union(fileQuery).union(sharedWorkflowFileQuery).fetch()
          case _ => throw new IllegalArgumentException("Unknown resourceType. Only 'workflow', 'project', and 'file' are allowed")
        }
