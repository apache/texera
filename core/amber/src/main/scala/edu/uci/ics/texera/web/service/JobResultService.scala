...
class JobResultService(
  ...
  def attachToJob(
    ...
    workflowInfo.getSourceOperators.map(source => {
      workflowInfo.getOperator(source) match {
    ...
    workflowInfo.getSinkOperators.map(sink => {
      workflowInfo.getOperator(sink) match {
