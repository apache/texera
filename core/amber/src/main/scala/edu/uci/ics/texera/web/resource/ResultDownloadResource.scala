...
object ResultDownloadResource {
  ...
  def apply(
    ...
    val response: ResultDownloadResponse = request.downloadType match {
      case "google_sheet" =>
        handleGoogleSheetRequest(request, results, attributeNames)
      case "csv" =>
        handleCSVRequest(request, results, attributeNames)
  ...
  def handleCSVRequest(
    ...
    val link: String = s"test_output.csv"
    val message: String =
      s"File saved to User Dashboard"
    ResultDownloadResponse(request.downloadType, link, message)
  }
