...
import edu.uci.ics.amber.engine.common.virtualidentity.{ExecutionIdentity, OperatorIdentity, PhysicalLinkIdentity, PhysicalOpIdentity, WorkflowIdentity}
...
object AggregateOpDesc {
  ...
  def getPhysicalPlan(
    ...
    new PhysicalPlan(
      ...
      links = Set(PhysicalLinkIdentity(partialPhysicalOp.id, 0, finalPhysicalOp.id, 0))
    )
  }
