...
class WorkflowPipelinedRegionsBuilder(
  ...
  private def getBlockingEdgesRemovedDAG(): PhysicalPlan = {
    val edgesToRemove = new mutable.MutableList[(OperatorIdentity, OperatorIdentity)]()
    ...
    physicalPlan.allOperatorIds.foreach(opId => {
      ...
      upstreamOps.foreach(upOpId => {
        val linkFromUpstreamOp = LinkIdentity(
          physicalPlan.operators(upOpId).topology.layers.last.id,
          physicalPlan.operators(opId).topology.layers.head.id
        )
        if (physicalPlan.operators(opId).isInputBlocking(linkFromUpstreamOp)) {
          edgesToRemove += ((upOpId, opId))
        }
      })
    })

    val linksAfterRemoval = physicalPlan.allLinks.filter(link => !edgesToRemove.contains(link))
    new PhysicalPlan(workflowId, physicalPlan.operators, toOutLinks(linksAfterRemoval))
  }
    ...
    physicalPlan
      ...
      .foreach(opId => {
        ...
        physicalPlan
          ...
          .foreach(upstreamOp => {
            val linkFromUpstreamOp = LinkIdentity(
              physicalPlan.operators(upstreamOp).topology.layers.last.id,
              physicalPlan.operators(opId).topology.layers.head.id
            )
            if (physicalPlan.operators(opId).isInputBlocking(linkFromUpstreamOp)) {
