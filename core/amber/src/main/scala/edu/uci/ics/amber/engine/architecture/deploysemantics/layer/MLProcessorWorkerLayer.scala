...
import edu.uci.ics.amber.engine.architecture.worker.{Generator, MLProcessor, Processor}
...
class MLProcessorWorkerLayer(
                              tag: LayerTag,
                              val metadata: Int => MLModelOpExec,
                              _numWorkers: Int,
                              df: DeploymentFilter,
                              ds: DeployStrategy
  ...
  override def clone(): AnyRef = {
    val res = new MLProcessorWorkerLayer(tag, metadata, numWorkers, df, ds)
