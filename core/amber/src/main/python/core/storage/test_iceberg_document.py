...
class TestIcebergDocument:
            ...
            Tuple(
                {
                    ...
                    "col-timestamp": None if i % 8 == 0 else datetime.datetime.fromtimestamp(
    ...
    def test_concurrent_writes_followed_by_read(self, iceberg_document, sample_items):
        ...
        item_batches = [
            all_items[i * batch_size + min(i, remainder): i * batch_size + min(i, remainder) + batch_size + (
                1 if i < remainder else 0)]
            for i in range(num_writers)
        ]

        assert (len(item_batches) == num_writers, f"Expected {num_writers} batches but got {len(item_batches)}")
        ...
        assert (set(retrieved_items) == set(all_items), "All items should be read correctly after concurrent writes.")
    ...
    def test_read_using_range(self, iceberg_document, sample_items):
        ...
        batch_size = 1500
        # Generate ranges
        ranges = [range(i, min(i + batch_size, len(sample_items))) for i in range(0, len(sample_items), batch_size)]

        # Retrieve items using ranges
        retrieved_items = [
            item
            for r in ranges
            for item in iceberg_document.get_range(r.start, r.stop)
        ]

        assert (len(retrieved_items) == len(sample_items), "The number of retrieved items does not match the number of "
                                                           "all items.")

        # Verify that the retrieved items match the original items
        assert (set(retrieved_items) == set(sample_items), "All items should be retrieved correctly using ranges.")
    ...
    def test_get_after(self, iceberg_document, sample_items):
        ...
        assert (not retrieved_items, (
            f"get_after({invalid_offset}) should return an empty list, but got: {retrieved_items}"
        ))
    ...
    def test_get_counts(self, iceberg_document, sample_items):
        ...
        assert(iceberg_document.get_count() == len(sample_items), "get_count should return the same number with "
                                                                  "allItems")
