...
class DataProcessor(StoppableQueueBlockingRunnable):
    ...
    def __init__(
        ...
        self._current_input_tuple_iter: Optional[Iterator[Union[Tuple,
                                                                InputExhausted]]] = None
        self._input_links: List[LinkIdentity] = list()
        self._input_link_map: MutableMapping[LinkIdentity, int] = dict()

        self.context = Context(self)
        self._async_rpc_server = AsyncRPCServer(
            output_queue, context=self.context)
        self._async_rpc_client = AsyncRPCClient(
            output_queue, context=self.context)
    ...
    def complete(self) -> None:
        ...
        control_command = set_one_of(
            ControlCommandV2, WorkerExecutionCompletedV2())
        self._async_rpc_client.send(
