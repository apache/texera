...
class TestTable:
    ...
    def test_table_creation(self, target_table, a_timestamp):
        ...
        assert target_table["field4"][0] == True
        assert target_table["field4"][1] == False
    ...
    def test_table_from_data_frame(self, target_table, a_timestamp):
        assert Table(pandas.DataFrame({
            "field1": [1, 2],
            "field2": ["hello", "world"],
            "field3": [2.3, 0.0],
            "field4": [True, False],
            "field5": [a_timestamp, datetime.datetime.fromtimestamp(1000000000)],
    ...
    def test_table_from_list_of_series(self, target_table, a_timestamp,
                                       target_raw_tuples,
                                       target_tuples):
        table = Table([
            pandas.Series(raw_tuple) for raw_tuple in target_raw_tuples
        ])
