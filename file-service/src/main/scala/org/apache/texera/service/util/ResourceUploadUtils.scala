...
/**
  * Upload helpers shared by every versioned file resource (datasets, models).
  *
  * These deal only with raw bytes and relative file paths, so they hold no
  * resource-specific state and can back any resource's upload flow.
  */
object ResourceUploadUtils {
  ...
  /**
    * Validates a file path using Apache Commons IO. Rejects empty paths, paths
    * that traverse above the root, and absolute paths.
    */
