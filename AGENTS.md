...
`amber/src/main/python/proto/` is gitignored and regenerated by
[`bin/python-proto-gen.sh`](bin/python-proto-gen.sh) on `sbt amber/compile`
(see `genPythonProto` in [`amber/build.sbt`](amber/build.sbt)). Requires
`protoc` on PATH (pin to `3.19.4` to match `PB.protocVersion`); skipped
with a warning when `protoc` is missing.
